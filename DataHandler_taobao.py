...
# Certifique-se de que Params.py esteja no mesmo diretório ou em um local acessível
...
def negSamp_v2(temLabel, sampSize, nodeNum):  # Renomeado para evitar conflito
    negset = [None] * sampSize
    cur = 0
    while cur < sampSize:
...
class DataHandler:
	...
	def LoadData(self):
    	...
    	def prepareGlobalData(self):
        	# A linha abaixo foi modificada para usar trnLabel diretamente.
        	# Comentários indicam que o código original somava várias matrizes, 
        	# mas a versão anterior usava apenas trnLabel.
        	adj = self.trnLabel.astype(np.float32)  # Usa trnLabel diretamente.
        	# adj = 0
        	# for i in range(len(self.behs)):
