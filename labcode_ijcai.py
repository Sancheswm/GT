...
class Recommender:
    ...
    def sequenceModeling(self, lats):
        ...
        embeds = (itemEmbeds + biasEmbed) * tf.expand_dims(tf.cast(tf.sign(self.ut_i_item), tf.float32), [-1])
        ...
        exp = tf.math.exp(logits) * (1.0 - tf.cast(tf.equal(logits, 0.0), tf.float32))
    ...
    def itemToUser(self, ilats):
        ...
        for i in range(args.behNums):
            mask = tf.cast(tf.expand_dims(tf.equal(self.ut_i_beh - 1, i), axis=-1), tf.float32)
    ...
    def aggregateSubUsers(self, utlat):
        ulat = tf.math.segment_sum(utlat, self.u_ut) / tf.expand_dims(tf.cast(tf.math.segment_sum(tf.ones_like(self.u_ut), self.u_ut), tf.float32) + 1e-6, axis=-1)
        return ulat
    ...
    def userToItem(self, utlats):
        ...
        for i in range(args.behNums):
            ...
            embed = tf.math.segment_sum(colLats, rows) / (tf.cast(tf.expand_dims(tf.math.segment_sum(ones, rows), axis=-1)) + 1e-6)
            behTrans_w = FC(tf.expand_dims(self.behEmbeds[i], axis=0), args.memosize, activation='relu', useBias=True, reg=True, name=paramId+'_a', reuse=True)
            behTrans = tf.reshape(FC(behTrans_w, args.latdim ** 2, reg=True, name=paramId+'_b', reuse=True), [args.latdim, args.latdim])
            embed = Activate(embed @ behTrans, self.actFunc)
            embeds.append(embed)
    ...
    def prepareModel(self):
        ...
        self.u_ut = tf.placeholder(name='u_ut', dtype=tf.int32, shape=[None])
        ...
        self.optimizer = Adam(learning_rate=learningRate)
        self.trainable_variables = tf.trainable_variables() # Define as variáveis treináveis
        ...
        for s in range(bigSteps):
            ...
            for i in range(steps):
                ...
                with tf.GradientTape() as tape:
                    pred = self.pred
                    sampNum = tf.shape(self.iids)[0] // 2
                    posPred = tf.slice(pred, [0], [sampNum])
                    negPred = tf.slice(pred, [sampNum], [-1])
                    preLoss = tf.reduce_sum(tf.maximum(0.0, 1.0 - (posPred - negPred))) / args.batch
    ...
    def sampleTestBatch(self, batIds, label, labelP, tstInt, u_i_dict):
        ...
        for i in range(batch):
            ...
            rdmNegSet = np.random.choice(negset, 99, replace=False)
    ...
    def testEpoch(self):
        ...
        for s in range(bigSteps):
            ...
            for i in range(steps):
                ...
                feeddict[self.utids] = uLocs
                feeddict[self.iids] = iLocs
                preds, atts = self.ours_test(feeddict) #self.sess.run([self.pred, self.atts], feed_dict=feeddict, options=config_pb2.RunOptions(report_tensor_allocations_upon_oom=True))
    ...
    def ours_test(self, feeddict):
        pred = self.ours()
        atts = [] #self.atts # Não precisa calcular atts no teste, economiza tempo
        with tf.Session() as sess: # Cria uma sessão local para executar o grafo
            sess.run(tf.global_variables_initializer()) # Inicializa as variáveis
            preds = sess.run(pred, feed_dict=feeddict) # Executa a predição
        return preds, atts
    ...
    def saveHistory(self):
        ...
        with tf.Session() as sess: # Salva o modelo em uma sessão local
            sess.run(tf.global_variables_initializer())
            saver.save(sess, 'Models/' + args.save_path)
        log('Model Saved: %s' % args.save_path)

    def loadModel(self):
        saver = tf.train.Saver()
        with tf.Session() as sess: # Carrega o modelo em uma sessão local
...
if __name__ == '__main__':
    ...
    recom = Recommender(handler) # Não precisa mais da sessão aqui
    recom.run()
